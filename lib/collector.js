...
function getOpts(opts) {
  ...
  program.option('-r, --repo-path <type>', 'Path to OCFL repository')
    ...
    .option('--sf-cache-path <file>', 'Optional path for Siegfried JSON cache file (defaults to dataDir/.siegfried.json)')
...
class Collector {
  ...
  constructor(opts = {}) {
    ...
    this.sfCachePath = this.opts.sfCachePath;
    this.sfData = new Map();
  }
  ...
  static async create(opts = {}) {
    ...
    if (collector.sf) {
      await collector.runSiegfried(undefined, collector.sfCachePath);
    }
    return collector;
  }

  async runSiegfried(force, sfCachePath) {
    // read the cache; fall back to dataDir/sfFileName if no explicit path given
    sfCachePath = sfCachePath || path.join(this.dataDir, this.sfFileName);
    let sfJson;
    try {
