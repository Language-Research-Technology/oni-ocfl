...
const { renderSinglePage } = require("ro-crate-html-lite");
...
class CollectionObject {
      ...
      if (entity['@type'].includes('File') && isRelFilePath(entityId)) {
        ...
        try {
          ...
          if (error.code === 'ENOENT') {
            file.exists = false;
            if (this.collector.logLevel === 'warning') {
  ...
  * @param {Array.<Array.<string>>} [files] If specified, each file in the array of tuple [source,destination][] will be imported to the OCFL object,
  ...
  async addToRepo(ignoreFilesInCrate = false, files) {
    ...
    //if (!this.crate.metadata.license?.[0]) throw new Error('Metadata file descriptor requires a license property.');
    try {
      // validate crate
      ...
      const previewContent = await renderSinglePage({crate: this.crate});
...
const validateWorksheet = {
  ...
  types(ws, roc, errors) {
    ...
    for (const e of roc.entities()) {
      if (e['@id'].includes('siegfried')) console.log(e['@id']);
